...
def format_response(response):
    # Function to format response based on content type
    if "```" in response:  # Code block
        response = response.replace("```", "<pre><code>").replace("```", "</code></pre>")
    elif "$$" in response:  # LaTeX math block
        response = response.replace("$$", "<div class='math'>").replace("$$", "</div>")
    else:
        # Replace newlines with <br> for general text formatting
